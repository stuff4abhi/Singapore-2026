...
- **Known limitation — opaque responses can't be read by page JS, only served by the service worker:** a `no-cors` cross-origin response is "opaque" — the Fetch API deliberately hides its body from script, by design, not as a bug. That means the cached file can *never* be turned into a `blob:` URL or otherwise read in `index.html`. The only way to actually use it is to have `sw.js` intercept a same-origin request (`./offline-file/?src=<drive url>`) and hand the browser the cached opaque response directly for a real navigation — the browser is allowed to render/download it even though script can't inspect it. `renderFiles()`'s "Open saved copy" link points at that proxy route; `markOfflineBadges()` only ever checks whether the cache has an entry, never reads it. Pinning (`pinEssentialFiles`) still happens page-side since `sw.js` can't intercept the outbound request to `drive.google.com` itself — only the later same-origin playback request.
