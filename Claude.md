...
- **Offline pinning (best effort, not guaranteed):** `pinEssentialFiles()` in `index.html` fetches each Essential file's Drive link with `mode:"no-cors"` and stores the response in the `sg2026-files-v1` Cache Storage bucket. This only works if the device already has an authenticated Google session with folder access and the browser doesn't block that session's cookie on a cross-site fetch — iOS Safari's ITP and Chrome's evolving third-party-cookie rules can silently prevent it. When pinning succeeds, a second "📌 Open saved copy" link appears; when it doesn't, the primary "Open in Drive" link is unaffected and is the reliable fallback.
- **Known limitation — opaque responses can only be shown as inline subresources, never navigated to or read by page JS:** a `no-cors` cross-origin response is "opaque" — the Fetch API deliberately hides its body from script (that's what "opaque" means, not a bug), always reporting status 0 with no readable headers. Browsers additionally reject a status-0 opaque response when it's used to answer a full-page **navigation** (e.g. a link with `target="_blank"`) — this was tried first and reliably produced "Site can't be reached" on a real device. What *does* work: rendering the opaque bytes as a same-origin **subresource** fetched from within the already-open page, e.g. an `<img>` tag. So "Open saved copy" is a button that opens an in-page lightbox (`#filePreview` in `index.html`, `openFilePreview()`/`closeFilePreview()`) whose `<img src>` points at `sw.js`'s `./offline-file/?src=<drive url>` proxy route, which answers straight from the `sg2026-files-v1` cache. This only works for **image files** — a non-image essential file will simply fail to decode (`img.onerror`) and show a "can't preview offline" fallback message instead. Pinning (`pinEssentialFiles`) still happens page-side since `sw.js` can't intercept the outbound request to `drive.google.com` itself — only the later same-origin `<img>` request.
